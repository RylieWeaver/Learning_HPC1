...
# Commands:
# - torchrun --standalone --nproc_per_node=2 test_all_gather.py  --tensor_parallel_size 2
# - nohup torchrun --standalone --nproc_per_node=2 test_all_gather.py  --tensor_parallel_size 2 > output.txt 2>&1 &
# - pkill -u "$(whoami)" -f 'test_all_gather.py'
...
class CPLinear(nn.Module):
    ...
    def forward(self, x):                                                       # [B, S, D_in]
        ...
        out = all_gather_tensor_autograd(                                       # [B, S, D_out]
            out, gather_dim=2, group=self.tp_group
        ) if self.gather_output and self.tp_size > 1 else out
...
if __name__ == "__main__":
    ...
    # Do one forward pass
    model.train()
    input_data = torch.empty(1, 10, input_dim, device=device)  # [B, S, D_in] where B=1, S=10
    labels = torch.empty(1, 10, output_dim, device=device)  # [B, S, D_out]
    if is_rank0():
        input_data = torch.randn(1, 10, input_dim).to(device)
        labels = torch.randn(1, 10, output_dim).to(device)
