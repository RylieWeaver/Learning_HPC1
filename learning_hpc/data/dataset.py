...
# Learning HPC1
from learning_hpc.distributed import rank0_write, is_rank0
...
class DNADataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        your chunk size is larger than your sequence length, but the dataset sequence size is 
        100M - how long of a context size are you testing?!?
        """
        worker = torch.utils.data.get_worker_info()
        base_seed = self.seed + (worker.id if worker is not None else 0)
        rng = random.Random(base_seed)
        while True:
            if is_rank0(self.parallel_state.sptp_group):
